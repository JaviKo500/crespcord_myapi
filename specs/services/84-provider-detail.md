...
- **Revisado:** 2026-08-14 — **`ratings[].unit` deja de ser el título del nodo `vivienda` y pasa a ser su nombre, `field_nombre_vivienda`**, el mismo valor que `GET /api/v1/units` devuelve como `name`. El título de una vivienda es una etiqueta interna del sitio y no la cadena por la que el residente reconoce su unidad, así que mostrarlo era el error. Sin fallback al título: una vivienda sin `field_nombre_vivienda`, o cuyo nodo ya no se puede cargar, responde `null` — por la misma razón, el título sería justo la cadena equivocada. Solo cambia `myapi_provider_build_rating_item()`; la consulta de `myapi_provider_ratings_recent()` no se toca.
- **Dependencias:**
...
- **`resources/provider.resource.inc`** (modificar) — el fichero ya anunciado por SPEC 82/83 como el lugar del detalle. Funciones nuevas, sin tocar ninguna de las nueve que ya existen:
  ...
  - `myapi_provider_build_rating_item($row)` — mapea una fila de calificación a `{stars, comment, author_name, unit, created}`, resolviendo el nombre abreviado del autor (perfil vía `myapi_user_fetch_profile_fields()`, con los mismos tres niveles de fallback que `myapi_claim_notification.inc`: perfil → `account->name` → "Usuario eliminado") y el nombre de la vivienda de `field_unit` (`field_nombre_vivienda`, no el título del nodo) vía `node_load()`, `NULL` si la calificación no la tiene o si la vivienda no tiene nombre registrado.
...
| `unit` | string \| **null** | Nombre (`field_nombre_vivienda`) del nodo `vivienda` de `field_unit` — el mismo valor que `GET /api/v1/units` devuelve como `name`, **no** el título del nodo. `null` si la calificación no tiene `field_unit` (todas las de hoy, hasta que exista el flujo que lo rellena), si la vivienda no tiene nombre registrado o si su nodo ya no existe; nunca cae al título |
...
Y por cada una de las (como mucho 3) calificaciones devueltas: un `user_load()` para el nombre y, si tiene `field_unit`, un `node_load()` para el nombre de la vivienda.
...
5. **`resources/provider.resource.inc` — `myapi_provider_build_rating_item($row)`.** El mapeo de una fila de calificación a las cinco claves, con la resolución del nombre abreviado (perfil → `account->name` → "Usuario eliminado") y del nombre de la vivienda (`field_nombre_vivienda`) vía `node_load()` cuando `field_unit` está presente, sin fallback al título. *Verificación: `php -l`; función aún no alcanzable.*
...
9. **Pruebas.**
   - `tests/unit/ProviderDetailEndpointTest.php` (nuevo): el dispatcher solo acepta `GET`; `401` sin cabecera y con token inválido; `404 provider_not_found` con `nid` inexistente, de otro tipo, despublicado y no numérico; un proveedor con la licencia vencida **sí** responde `200` (a diferencia del listado); la forma exacta de las trece claves, en orden; `categories`, `rating_avg`, `rating_count`, `short_description` y `hourly_rate` calculados igual que en el listado para el mismo proveedor (cruce de valores); `address` y `description` vía `myapi_text_to_plain()` con una entrada que trae `<p>` y `&nbsp;`; `tags` como array de strings y `[]` sin ninguno; `gallery` con las mismas tres claves e igual orden que `GET /api/v1/providers/%/gallery` para el mismo proveedor; `ratings` vacío y `rating_summary` en ceros para un proveedor sin calificaciones; las últimas 3 en orden de `created` descendente cuando hay más de tres; `rating_summary` sumando el histórico completo y no solo las tres mostradas; `unit: null` cuando la calificación no tiene `field_unit`, cuando la vivienda no tiene `field_nombre_vivienda` y cuando su nodo no se puede cargar; `unit` con el nombre de la vivienda (no su título) cuando sí lo tiene; el nombre abreviado con sus tres niveles de fallback (perfil, username, cuenta borrada).
...
- [x] Una calificación **con** `field_unit` responde el **nombre** (`field_nombre_vivienda`) del nodo `vivienda` referenciado, no su título. *(Revisión 2026-08-14: el criterio original pedía el título; `testUnitIsNullWithoutFieldUnitAndTheUnitNameWithIt` siembra una vivienda cuyo título difiere de su nombre para que leer el equivocado no pueda pasar.)*
- [x] **(nuevo, 2026-08-14)** Una vivienda **sin** `field_nombre_vivienda`, o cuyo nodo ya no se puede cargar, responde `unit: null` — nunca cae al título.
- [ ] **(nuevo, 2026-08-14, no cubierto por test)** `author_name` y `unit` viajan como texto plano sin escapar, igual que el resto de la respuesta. *Cerrado solo por inspección: `myapi_provider_build_rating_item()` llama a `myapi_text_to_plain()` en las dos claves y conserva el ternario de `unit` para que una calificación sin vivienda siga respondiendo `null` y no `""`. Ningún test de `ProviderDetailEndpointTest` siembra un `&` ni un `<b>` en un nombre de perfil ni en un nombre de vivienda, así que la suite pasaría igual si alguien devolviera el `check_plain()`. Falta un test con esos literales.*
