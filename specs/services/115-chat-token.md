...
- **Ni un campo, ni una tabla, ni un `hook_update_N`, ni una fila escrita.** Los tres campos de SPEC 77 **siguen vacíos al terminar este spec**. La ruta del hilo es una **convención** derivada del `nid` de la oferta, no un dato almacenado — ver Decisión 8. **✅ Resuelto por SPEC 117:** este endpoint es hoy el escritor de `field_firebase_path`, que se llena la primera vez que una credencial cubre el hilo. Lo que **no** ha cambiado es lo que esta nota defiende: la ruta se sigue derivando del `nid` en cada firma y la **Decisión 8 queda intacta** — lo que se escribe es un espejo de sólo lectura para el back office, no una fuente. Ver `specs/services/117-chat-fields-mirror.md`.
...
- **Abrir el hilo, y escribir los tres campos de SPEC 77.** `field_firebase_path`, `field_chat_opened_at` y `field_last_message_at` **siguen vacíos**. Con la ruta por convención (Decisión 8) el chat funciona sin ellos; escribirlos es lo que hará falta el día que el back office tenga que ver el hilo o que la ruta deje de ser derivable. **✅ Resuelto por SPEC 117**, y por la primera de las dos razones que esta viñeta anticipaba: el back office. La ruta **no** ha dejado de ser derivable, así que de las dos mitades se cumple una sola — se escriben los tres campos (`field_firebase_path` aquí, los dos *timestamps* en el `notify` del 116) y el chat sigue funcionando exactamente igual sin ellos, porque **nadie los lee**. Ver `specs/services/117-chat-fields-mirror.md`.
...
| **Nada avisa de un mensaje nuevo.** Sin el spec hermano, el chat solo se ve si abrís la app. · **✅ Resuelto por SPEC 116:** el cliente que escribe llama a `POST /api/v1/chat/threads/{offer_nid}/notify` y el otro lado recibe un banner que dice quién escribió y sobre qué solicitud —nunca qué—, con *debounce* de un aviso por hilo y destinatario cada 60 s. Ver `specs/services/116-chat-message-push.md` | Era la primera cosa que había que hacer después de esta, y lo fue. **Queda un resto asumido:** el aviso depende de que el cliente llame, así que si el móvil que escribió se muere entre el `set()` de Firebase y el `POST`, el mensaje llega y el banner no. |
