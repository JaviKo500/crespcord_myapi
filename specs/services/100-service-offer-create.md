...
- **Editar o retirar una oferta** (`PUT` / `DELETE`, el estado `withdrawn`). Este spec crea, y nada más. Es una limitación real, anotada en Riesgos. **✅ Resuelto por SPEC 105:** dos rutas nuevas, `PUT /api/v1/service-offers/{id}` y `.../withdraw`; el `DELETE` se descartó a propósito, porque la oferta retirada no desaparece. Ver `specs/services/105-service-offer-update-withdraw.md`.
...
| Riesgo | Mitigación |
|---|---|
| **1. El proveedor no puede corregir su oferta.** La unicidad la bloquea en `sent` y no hay `PUT` ni `DELETE`: un cero de más en el monto se queda escrito hasta que el residente adjudique a otro o cancele. · **✅ Resuelto por SPEC 105:** existen `PUT /api/v1/service-offers/{id}` —reemplazo total mientras la oferta siga `sent`— y `PUT /api/v1/service-offers/{id}/withdraw`, que es lo que por fin escribe `withdrawn`. Ver `specs/services/105-service-offer-update-withdraw.md` | Asumido y explícito **mientras duró**. Fue el precio de partir el trabajo en dos specs, y la salida —`PUT` y el estado `withdrawn`, que ya existe en el catálogo desde SPEC 77— estaba diseñada aunque no implementada. Debía ser el spec inmediatamente siguiente, y lo fue. El consejo de confirmar el envío con un resumen sigue en pie: corregir es mejor que retirar y reofertar, porque lo segundo deja dos nodos en el listado del residente. |
