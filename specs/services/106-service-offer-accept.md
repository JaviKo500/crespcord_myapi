...
- **Adjudicar sobre una solicitud `direct`.** Adjudica en el nacimiento (SPEC 87) y no tiene ofertas que aceptar; el grafo no lleva `direct → assigned` y este spec no se la añade. **✅ Resuelto por SPEC 107**, que sí añadió esa arista y dejó que el residente aceptara el presupuesto por este mismo endpoint, y **reasignado por SPEC 120**, que la recorre desde el `POST` de la oferta: sobre una `direct` este endpoint responde hoy `409 service_offer_not_acceptable`, porque la oferta ya es `selected` antes de que el residente llegue a ella.
