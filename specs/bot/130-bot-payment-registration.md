...
### 2. `hook_schema()` + los `hook_update_N()` en `myapi.install`

La tabla `myapi_bot_payments` de la sección anterior, en `hook_schema()` (para instalaciones nuevas) y en un `hook_update_N()` con `db_create_table()` guardado por `db_table_exists()` (para el sitio que ya corre). **Esta spec no se despliega sin la SPEC 129 aplicada.**

Son **dos** hooks, y el segundo es la cicatriz de un incidente:

- **`myapi_update_7048()`** — crea la tabla. El número no es el `7047` que decía este plan: entre redactarse y implementarse, el `7047` lo ocupó la reparación del backfill de la SPEC 129.
- **`myapi_update_7049()`** — añade la columna `uid_resolved`, que llegó con la revisión que hizo opcional `identity.person.uid`.

**Por qué la columna no va dentro del `7048`,** aunque sea la misma tabla y parezca más limpio: se intentó, con un `db_field_exists()` delante que *parecía* idempotente. No lo es — es código inalcanzable. `drush updb` solo ofrece los hooks numerados **por encima** de la `schema_version` que el sitio guarda, así que un sitio que ya corrió el `7048` no vuelve a verlo nunca y nada de lo que haya dentro se ejecuta. El resultado en dev fue una caída: el código desplegado escribía una columna que la base no tenía, y **todos** los pagos del bot fallaron —también los que funcionaban— hasta que existió el `7049`.

La regla, para que no se reaprenda una tercera vez: **un `hook_update_N()` ya aplicado es historia del sitio y es inmutable.** Todo cambio de esquema lleva número nuevo, cueste lo que cueste en pulcritud. Hay un test que lo fija (`testTheColumnIsAddedByItsOwnHookAndNotByEditingTheOldOne`).
...
| **Archivo huérfano cuando la transacción revierte.** El comprobante se guarda como managed file **antes** de la transacción (paso 11); si el `INSERT` falla y se revierte, el archivo queda en disco sin nodo y sin fila en `file_usage`. | Se asume, y **la mitigación que este spec daba era falsa**. Decía que «el cron de Drupal lo recoge»: no lo hace. `file_cron()` de Drupal 7 borra ficheros **temporales** viejos, y `myapi_payment_save_file()` guarda el comprobante como `FILE_STATUS_PERMANENT`. Un permanente sin uso **no lo borra nadie**: se queda en `file_managed` y en `private://comprobantes_pago/` hasta que alguien lo barra a mano. La ventana tampoco es estrecha: no es solo la carrera de dos reintentos, es **cada fallo de la transacción**, y un fallo sistemático —como el de la columna `uid_resolved` ausente— deja un huérfano por intento. Corregido tras el incidente de dev descrito en `myapi_update_7049()`. Queda como deuda conocida: si se quiere cerrar, el arreglo es mover el guardado del archivo dentro de la transacción o borrarlo explícitamente en el `catch`. |
