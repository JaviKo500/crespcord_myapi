...
| `<comentario>` | `field_comment`, whitespace collapsed to one line and cut to 60 characters with core's `truncate_utf8($c, 60, TRUE, TRUE)` — word-safe, ending in `…` | Dropped (the normal case for the automatic initial transaction) |
