...
| File | Role |
|------|------|
| `myapi.module` | `hook_menu()` entry for the creation route, `myapi_claim_transaction_add_access()`, `myapi_form_reclamo_node_form_alter()` and `myapi_form_claim_transaction_node_form_alter()` (glue only, both delegate), the `'reclamo'`/`'claim_transaction'` branches of `myapi_node_insert()`/`myapi_node_update()`, and the `'claim_transaction'` branch of `myapi_node_presave()` (SPEC 60, the autogenerated title). |
| `includes/myapi.claim_transaction_admin.inc` | Everything else: the timeline query and its render, the "Editar" link and its access check, the creation form and its submit handler, the creation page callback, the edit-form alter and its redirect handler, the autogenerated title, the automatic initial transaction, and the status sync. Loaded by the `file` key of the menu entry and by `module_load_include()` from `myapi.module`. |
...
| `myapi.install` | Configuration of `field_status_date`: `_myapi_claims_install()` declares it with `hour`/`minute` granularity and a `'Y-m-d H:i'` `date_select` widget, and `myapi_update_7019()` (SPEC 58) applies that same configuration to sites installed earlier — no row of `field_data_field_status_date` is ever rewritten. Plus `myapi_update_7020()` (SPEC 60), the only data update of this feature: it fills in the empty `node.title` of the transactions saved before that spec. |
...
`drush updb` is needed since SPEC 60 (`myapi_update_7020()`, the title
backfill). No field, table or permission is added by any of these specs — the
...
Every `claim_transaction` is titled by the module, never by the operator.

```
Reclamo #128 · En proceso · 03/08/2026 14:30 · Se contactó al proveedor para agendar la…
```

**Why it exists.** The bundle has a native title (`has_title = 1`, label
"Título"), but neither of the module's own creation paths wrote it: the
creation form's submit handler (above) and
`myapi_claim_transaction_create_initial()` (below) both build the node field by
field and never touched `$transaction->title`. `node.title` is `NOT NULL
DEFAULT ''`, so `node_save()` stored an empty string instead of failing — a
blank link in `/admin/content` and no way to tell one transaction from another
outside the claim's timeline.

**Format.** Four segments joined by `' · '`, built by
`myapi_claim_transaction_title($claim_nid, $status_label, $status_date, $comment)`.
**A segment whose value is missing is dropped**, with no dangling separator:

| Segment | Source | Missing |
|---|---|---|
| `Reclamo #<nid>` | `field_claim` (`myapi_building_admin_field_target_id()`) | Dropped |
| `<Estado>` | `field_status`, labelled from `myapi_claims_status_options()` (SPEC 56), **raw** — an unknown value falls back to the stored value | Dropped |
| `<d/m/Y H:i>` | `field_status_date` via `format_date(strtotime(...), 'custom', …)`; an unparseable value is dropped rather than printed as 1970 | Dropped |
| `<comentario>` | `field_comment`, whitespace collapsed to one line and cut to 60 characters with core's `truncate_utf8($c, 60, TRUE, TRUE)` — word-safe, ending in `…` | Dropped (a transaction saved from the native node form with the textarea empty; since SPEC 61 the automatic initial transaction always has one) |

With no segment at all the title is `Transacción de reclamo`: a transaction is
never titleless again. The whole string is capped at 255, `node.title`'s column
width.

Nothing is escaped on the way in. `node.title` is stored raw and escaped by
Drupal at render time, which is why the status label comes from
`myapi_claims_status_options()` and **not** from `myapi_claims_status_label()`
— that one returns it `check_plain()`'d for the timeline's HTML table, and
storing `&amp;` in the column would double-escape it on screen. Same catalogue,
different consumer.

**Where it is generated.** `myapi_claim_transaction_set_title($node)`, called
from `myapi_node_presave()`'s `'claim_transaction'` branch. `hook_node_presave()`
and not the form submit handlers:

- `field_attach_presave()` runs **before** it inside `node_save()`, so
  `field_status_date` is already normalised to its stored `'Y-m-d H:i:s'`
  whatever widget produced it; and the row is not written yet, so no second
  save is needed to set the title.
- One implementation covers **all four** paths: the creation page (SPEC 57),
  the automatic initial transaction, `node/add/claim_transaction` and
  `node/%nid/edit` (SPEC 59).
- Because it also runs on update, the title is **regenerated** whenever the
  operator edits status, date or comment — it can never describe a state the
  transaction no longer has.

**The native "Título" field is hidden.** `myapi_claim_transaction_transaction_form_alter()`
sets `#access = FALSE` and `#required = FALSE` on `$form['title']`, in creation
**and** edition — the only part of that alter that runs above its edit-mode
guard. Anything typed there would be overwritten on save, so the field is not
shown. Dropping `#required` matters: `_form_builder_handle_input_element()`
derives `$process_input` from `#access`, so an inaccessible element is never
validated, and the explicit `FALSE` states that intent instead of relying on
it.

**Transactions saved before this spec** are filled in by `myapi_update_7020()`
(`drush updb`), which composes the title with the same function and writes
`node.title` **and** `node_revision.title` with `db_update()` — not
`node_load()` + `node_save()`, which would fire the status sync below and leave
every claim carrying the status of whichever of its transactions was rewritten
last. Only rows with `title = ''` are touched: a title an operator typed by hand
on `node/add/claim_transaction` is left alone and converges to the automatic
...
$transaction->field_comment[LANGUAGE_NONE][0]['value']    = myapi_claim_transaction_initial_comment(myapi_building_admin_field_value($node, 'field_claim_type'));
```
...
### Acknowledgement comment (SPEC 61)

`field_comment` is not empty any more: `myapi_claim_transaction_initial_comment($claim_type)`
composes the text the timeline's first row shows, and the noun follows
`field_claim_type`:

| `field_claim_type` | Comment |
|---|---|
...
| missing / unknown | Same as `claim` — the field is required on the bundle, so this is corrupt data, and the row still gets a comment rather than the `—` this spec removes |

The noun is written in that function, **not** lowercased off
`myapi_claims_claim_type_options()`: that catalogue supplies select labels
("Reclamo", "Requerimiento") and editing an `allowed_value` from the UI would
silently rewrite the sentence mid-phrase. Labels come from the field, meaning
does not — the same split `myapi_claims_valid_claim_type()` already states for
the stored values (SPEC 56).

Only `value` is written, no `format`, exactly like the creation form's submit
handler: the timeline reads the raw column and `check_plain()`s it itself.

Side effect, stated rather than discovered later: the transaction is no longer
commentless, so its autogenerated title (SPEC 60) gains its fourth segment —
`Reclamo #128 · Recibido · 03/08/2026 14:30 · Hemos recibido su reclamo. Será revisado por la…`.

Claims created **before** this spec keep their empty initial comment. No
`hook_update_N` backfills them: the comment is a message written at reception
time, and back-dating one onto old timelines would state something that never
happened.
...
is a field read plus an assignment.
`tests/unit/ClaimTransactionTitleTest.php` (SPEC 60) covers
...
| Case | Expected |
|---|---|
| Create from `claim-transaction/add` | Title `Reclamo #<claim_nid> · <Estado> · <d/m/Y H:i> · <comentario truncado>` in `/admin/content` |
| Create a `reclamo` | Its automatic initial transaction is titled too, with three segments (no comment) |
| Create from `node/add/claim_transaction` | No "Título" field on the form; saves without asking, same automatic format |
| Edit status / date / comment | Title regenerated with the new values |
| Comment longer than 60 characters | Cut on a word boundary, ending in `…` |
| Comment with line breaks | Title stays on one line |
| Transaction with no claim, status, date or comment | Title `Transacción de reclamo`, never empty |
| `node/%nid/edit` of a `reclamo` | Unaffected — its title ("Asunto") is still typed by hand |
| `drush updb` | `myapi_update_7020()` reports how many transactions it filled in; already-titled ones untouched; no claim changes `field_status` |
| Re-run `drush updb` | Nothing left to do, no error |
