...
| `channel` / `field_canal` | — | — | **Ignored.** Always forced to `"app"`. The channel is decided by the door the request came in through, never by the caller. |
| `field_banco_emisor` / `field_banco_destino` / `field_comprobante_ocr` | — | — | **Ignored.** Never accepted from this endpoint; they stay empty on every payment the app creates. See [Internal node fields](#internal-node-fields-not-exposed-by-any-endpoint). |
...
= 1`, `field_estado_pago = "Pendiente de verificar"`, `field_canal = "app"`.

**Side effect — email to the back office.** Once the payment is committed, the
detail email `payment_created_admin` is **enqueued** for every active user
holding the `backend` role, with a `Revisar pago` button pointing at
`node/{nid}/edit`. It is deferred (sent by the `myapi_mail_send` queue worker
on the next cron run) and best-effort: nobody holding the role, or a failure to
enqueue, changes nothing about the `201`. Full contract in
[payment-workflow.md](payment-workflow.md#email-to-the-back-office-on-registration).
...
## Internal node fields (not exposed by any endpoint)

Four fields of the `pagos` bundle exist for traceability and for the back
office, and **none of them is returned by any endpoint of this module**. They
are documented here so nobody looks for them in a response, and so nobody adds
them to one by accident.

| Field | Drupal type | Written by | Holds |
|-------|-------------|------------|-------|
| `field_canal` | `list_text`, single value | `POST /api/v1/payments` (`app`); the WhatsApp bot endpoint (`bot`); the back office form, whose default it is (`backoffice`) | The door the payment came in through. `allowed_values`: `bot` → «Bot de WhatsApp», `app` → «App», `backoffice` → «Back office». |
| `field_banco_emisor` | `text` (255), plain | The bot endpoint | Name of the bank the money left, as the OCR read it off the receipt. Free text: it is **not** validated against the `bancos` vocabulary, because that vocabulary holds the condominium's own accounts and not the national bank catalogue. |
| `field_banco_destino` | `text` (255), plain | The bot endpoint | Name of the bank the money arrived at. Same rules as the previous one. |
| `field_comprobante_ocr` | `text_long`, plain | The bot endpoint | The JSON of the receipt reading, as internal evidence. Stored with `text_processing = 0` so it comes back byte for byte, and `hidden` in every display: it is third-party content and the operator reads it in the node edit form's textarea. |

- **`field_banco` is untouched by all four.** It remains the term reference to
  the `bancos` vocabulary the app fills, with the same semantics and the same
  `invalid_bank` validation. The two new bank fields are plain text and live
  beside it; they do not replace it.
- **No endpoint filters or searches by channel.** There is no `?channel=`
  parameter on any listing.
- **Payments created before these fields existed** were all marked
  `backoffice` by `myapi_update_7046()` — there is no reliable way to tell an
  app payment from a hand-typed one after the fact — and their other three
  fields are empty.
- The only place the channel becomes visible is the back-office email, as the
  `Canal` line. See
  [payment-workflow.md](payment-workflow.md#email-to-the-back-office-on-registration).
